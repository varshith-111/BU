...
const fetchArticlesByCategory = async (category: string, id: string): Promise<NewsItem[]> => {
  ...
  return new Promise((resolve, reject) => {
    const req = request(`${baseUrl}/api/Articles/GetByCategory/${category}`, { agent }, (res) => {
      let data = '';
...
export default async function ArticleLayout({ 
  ...
  const relatedArticles: NewsItem[] = await fetchArticlesByCategory(category, id);
