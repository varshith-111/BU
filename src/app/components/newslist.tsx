...
export default function NewsList() {
  return (
    ...
    {newsItems.map((item) => (
      <Link key={item.id} href={`/article/${item.id}/${encodeURIComponent(item.title)}`} className="news-item">
        <img src={item.imageUrl} alt={`News Image for ${item.title}`} />
        <div className="news-content">
          <h3>{item.title}</h3>
          <p>{item.timeAgo} • {item.views.toLocaleString()} views</p>
        </div>
      </Link>
    ))}
  </section>
